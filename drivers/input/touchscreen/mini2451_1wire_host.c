...
#define GPIO_1WIRE			S3C2410_GPG(2)
...
static int init_timer_for_1wire(void)
{
	...
	// timer 2/3/4 input clock rate
	rate /= ((prescale1_value + 1) << 1);

	// calc the TCNT_FOR_SAMPLE_BIT, that is one of the goal
	TCNT_FOR_SAMPLE_BIT = rate / SAMPLE_BPS - 1;
	TCNT_FOR_FAST_LOOP  = rate / FAST_LOOP_FEQ - 1;
	TCNT_FOR_SLOW_LOOP  = rate / SLOW_LOOP_FEQ - 1;

	// config timer 3 as 1/2
	tcfg1 = __raw_readl(S3C2410_TCFG1);
	tcfg1 &= ~S3C2410_TCFG1_MUX3_MASK;
	writel(tcfg1, S3C2410_TCFG1);

	printk("1-wire Timer: TCNT = %ld, TCFG1 = %08x\n",
