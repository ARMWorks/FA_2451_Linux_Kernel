...
static void s3c2450_nand_read_buf(struct mtd_info *mtd, u_char *buf, int len)
{
	void __iomem *regs = s3c_nand.regs;
...
static void s3c2450_nand_write_buf(struct mtd_info *mtd, const u_char *buf, int len)
{
	void __iomem *regs = s3c_nand.regs;
...
{	
	...
	for (i = 0; i < sets->nr_chips; i++) {
		...
		if (!type->pagesize) {
			if (((nand->cellinfo >> 2) & 0x3) == 0) {
				...
				if ((2048 << (tmp & 3)) < 4096) {
					...
					printk("Unsupported MLC NAND flash.\n");
					goto exit_error;
				}
			}
		} else {
		...
		if (cpu_type == TYPE_S3C2450) {
			nand->read_buf	= s3c2450_nand_read_buf;
			nand->write_buf	= s3c2450_nand_write_buf;
		}
...
static int __init s3c_nand_init(void)
{
	...
	printk("                 (c) 2012 FriendlyARM (www.arm9.net)\n");

	platform_driver_register(&s3c6410_nand_driver);
	return platform_driver_register(&s3c2450_nand_driver);
}
