...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0))
static inline struct yaffs_linux_context *yaffs_sb_to_lc(struct super_block *sb)
{
	struct yaffs_dev *dev = yaffs_super_to_dev(sb);

	return yaffs_dev_to_lc(dev);
}

static void yaffs_sb_sync_dwork_func(struct work_struct *work)
{
	struct delayed_work *dwork;
	struct yaffs_linux_context *ctxt;

	dwork = container_of(work, struct delayed_work, work);
	ctxt = container_of(dwork, struct yaffs_linux_context, sb_sync_dwork);

	if (ctxt->sb_dirty)
		yaffs_write_super(ctxt->super);
}

static void yaffs_init_sb_sync_dwork(struct yaffs_linux_context *ctxt)
{
	spin_lock_init(&ctxt->sb_sync_dwork_lock);
	INIT_DELAYED_WORK(&ctxt->sb_sync_dwork, yaffs_sb_sync_dwork_func);
}

static void yaffs_cancel_sb_sync_dwork(struct super_block *sb)
{
	struct yaffs_linux_context *ctxt = yaffs_sb_to_lc(sb);

	cancel_delayed_work_sync(&ctxt->sb_sync_dwork);
}
...
static inline void yaffs_sb_set_dirty(struct super_block *sb, int dirty)
{
	...
	if (ctxt->sb_dirty == dirty)
		return;

	spin_lock(&ctxt->sb_sync_dwork_lock);

	ctxt->sb_dirty = dirty;
	if (dirty) {
		queue_delayed_work(system_long_wq, &ctxt->sb_sync_dwork,
				msecs_to_jiffies(dirty_writeback_interval * 10));
	}

	spin_unlock(&ctxt->sb_sync_dwork_lock);
}
#else
static inline bool yaffs_sb_is_dirty(struct super_block *sb)
{
...
static inline void yaffs_init_sb_sync_dwork(struct yaffs_linux_context *ctxt) {}
